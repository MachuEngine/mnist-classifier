...
이 프로젝트는 MNIST 데이터셋을 사용하여 손글씨 숫자(0~9)를 분류하는 합성곱 신경망(CNN) 모델 및 LeNet5 아키텍처를 구현하고, 교차 검증 및 하이퍼파라미터 최적화를 통해 결과를 비교 분석합니다.
...
- 교차 검증(K-Fold)을 통해 모델의 일반화 성능을 평가합니다.
- Optuna를 사용하여 하이퍼파라미터 최적화를 수행합니다.
- 학습률 스케줄러를 도입하여 학습의 안정성과 효율성을 향상시킵니다.
- 학습된 모델을 테스트 데이터로 평가합니다.
- 예측 결과 및 평가 지표를 시각화합니다.
- CNN과 LeNet5 모델 간 결과를 비교합니다.

---

## 모델 아키텍처
### CNN 모델
- 세 개의 합성곱 층과 풀링 층을 포함
...
## 추가 개발 방향
- 더 깊은 신경망 구조 실험: 더 많은 합성곱 층과 완전 연결 층을 추가하여 모델의 성능을 향상시킬 수 있습니다.
- 데이터 증강 기법 확장: 추가적인 데이터 증강 기법을 도입하여 모델의 일반화 성능을 더욱 향상시킬 수 있습니다.
- 다른 데이터셋 적용: Fashion-MNIST, CIFAR-10 등 다른 데이터셋에 모델을 적용하여 성능을 비교 분석할 수 있습니다.
- 학습률 스케줄러 다양화: 다양한 학습률 스케줄러를 시도하여 최적의 학습률 조정 전략을 찾을 수 있습니다.
- 다양한 옵티마이저 실험: SGD, RMSprop 등 다른 옵티마이저를 사용하여 모델의 성능을 비교할 수 있습니다.
- 교차 검증 방법 개선: Stratified K-Fold 등 다른 교차 검증 방법을 도입하여 클래스 불균형을 고려할 수 있습니다.
